...
**Status**: partial
**Category**: core
**実装率**: 57% (W-1 done, W-2 done, W-3 done, W-5 done)
...
**推奨順序（更新）**:
- 実装済み: W-1, W-5
- 次候補: W-2 → W-7 → W-3 → W-6 → W-4

理由:
- W-1（完了）で川が自然な方向に流れるようになり、全プリセットの見た目基盤が改善
- W-2 で海岸線の個性が出るため、探索の発見スポットを先に増やせる
- W-7 で水辺に適切な建物が出ると、W-2/W-6の入り江・デルタが「生きた場所」になる
- W-3 は低コストで河川の説得力を補強できる
- W-6 は W-7 まで揃ってから着手すると探索価値を作りやすい
- W-4 はシェーダー変更を伴うため最後に回す
