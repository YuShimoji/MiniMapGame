...
また、地勢だけでは不足する「上空観測密度」の扱いは SP-041 を参照する。
SP-040 は思想・制約・責務分離・評価軸・ロードマップを扱い、SP-032 は relief layer の実装仕様、SP-041 は observed surface layer の実装方針を扱う。
...
### 3.5 ビジュアル品質基準と検証優先順位

本プロジェクトの画作りは Unity ランタイム上のプレイ体験を最終ターゲットとする。`browser-preview` はその前段で使う**高速な方向確認用プローブ**であり、Unity 表示の代用品でも shader 実装の SSOT でもない。

#### 2026-04 corrective note

直近の問題は、browser-preview が「SP-040 の visual direction を検証する道具」ではなく、「GridGround.shader の 2D 直訳」に寄ってしまったことだった。  
このため hillshade / contour / influence tint は実装されていても、結果は最終成果物の品質軸に接続していなかった。

browser-preview に求めるもの:
- 地図としての可読性を高速に見ること
- 面・線・塊の優先順位を決めること
- Unity に持ち帰るべきパレット / コントラスト / レイヤリング方針を決めること

browser-preview に求めないもの:
- shader の step を 1:1 で再演すること
- Unity と完全一致の見た目を作ること
- ここだけで最終合否を決めること

**検証優先順位**:
1. browser-preview (Canvas 2D) で 4 preset × 2 theme の可読性と印象設計を確認
2. 確立した色パレット・レイヤリング方針を Unity shader / material parameter に移植
3. Unity PlayMode で 3D 固有の問題 (Z-fighting, LOD, パフォーマンス, カメラ距離差) を検証

**品質評価の基準**:
- 「フルスクリーンで派手か」ではなく、「スクリーンショットや縮小表示でも地図として読めるか」
- 道路ネットワーク・地勢・水辺・建物密度の優先順位が一目で分かるか
- 画面全体が同じコントラスト帯に沈まず、主役と背景のレイヤが分離しているか

### 3.6 Relief だけでは足りない

`hybrid orthophoto relief` は北極星として維持するが、それだけではユーザーが求める
「森林の厚み」「人工地形の用途感」「住宅地の異様な密度」は出ない。

したがって SP-040 配下では、次の 2 層を明確に分ける。

- **relief layer**: 地勢・標高・陰影・等高線
- **observed surface layer**: 林冠密度・人工面・屋根群・河岸帯・用途痕跡

前者は SP-032、後者は SP-041 で扱う。
...
- **SP-041: Observed Surface Synthesis** — forest / river / yard / housing の「上空観測密度」を Field / Grammar / Glyph で合成する
